...
```
E.g. To save in the 'ExperimentPlatform' folder with file name 'new_map':
```
rosrun map_server map_saver -f  /home/sinapse/ExperimentPlatform/new_map
```
4. A .pgm and .yaml file will be generated, transfer them to the `ExperimentPlatform` folder if it is not already done so
...
7.	To create a new reward map, create a new csv file in the 'RewardData' folder. In the GUI window, choose the new csv file from the drop-down list. Use the G key in Rviz to define the poster locations/direction (click and drag). The csv file will update the data accordingly.
